...
export async function onRequestPost(ctx: any) {
  try {
    ...
    if (!FILES) {
      return Response.json({ error: 'R2-Binding "FILES" nicht konfiguriert. Bitte im Cloudflare Pages Dashboard unter Settings → Functions → R2 Bucket Bindings eintragen.' }, { status: 503 });
    }
